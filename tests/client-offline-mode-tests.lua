...
function tests.testClientCreationOffline()
  print("Running test: Client creation in offline mode")
  ...
  assertTrue(client:IsReady(), "Client should be ready immediately in offline mode")
end
...
function tests.testFeatureFlagEvaluation()
  ...
  local client = Client.New(ClientConfigBuilder.New("test-app")
    :Offline(true)
    :Bootstrap(sampleBootstrap)
    :Build())
...
function tests.testBootstrapOverrideOffline()
  ...
  local client2 = Client.New(ClientConfigBuilder.New("test-app")
    :Offline(true)
...
function tests.testEmptyBootstrapOffline()
  ...
  assertTrue(client:IsReady(), "Client should be ready with empty bootstrap")
...
function tests.testErrorHandlingOffline()
  ...
  assertError("bootstrap must be a table", function()
    Client.New(ClientConfigBuilder.New("test-app")
      :Offline(true)
...
function tests.testMultipleClientsOffline()
  ...
  local client1 = Client.New(ClientConfigBuilder.New("app1")
    :Offline(true)
    :Bootstrap({
      {
        name = "feature-1",
  ...
  local client2 = Client.New(ClientConfigBuilder.New("app2")
    :Offline(true)
    :Bootstrap({
      {
        name = "feature-2",
...
function tests.testDelayedBootstrapOffline()
  ...
  assertTrue(client:IsReady(), "Client should be ready even without bootstrap")
...
function tests.testCustomLoggerOffline()
  ...
  for _, log in ipairs(logMessages) do
    if log.level == "info" and string.match(log.message, "Client initialized in offline mode") then
      foundInitLog = true
      break
    end
  end
...
function tests.testCustomStorageOffline()
  ...
  assertTrue(client:IsReady(), "Client should be ready with custom storage")
